    ...
    <div className={styles.page}>
                ...
                {coverPhotos.map((photo: any, i: number) => (
                  <div key={photo.id || i} className={styles.coverPhoto}>
                    {photo.url?.match(/\.(mp4|webm|ogg|mov)$/i) ? (
                      ...
                      <Image
                        src={photo.imageData || getSafeUrl(photo.url || "")}
                        alt={album.name}
                        fill
                    ...
                    <div className={styles.albumStackInner}>
                      {child.photos && child.photos.length > 0 ? (
                        child.photos[0].url?.match(/\.(mp4|webm|ogg|mov)$/i) ? (
                          ...
                          <Image
                            src={child.photos[0].imageData || getSafeUrl(child.photos[0].url || "")}
                            alt={child.name}
                            fill
                            className={styles.albumCoverImg}
                            sizes="150px"
      ...
      {isOwner && (
        <div className={styles.fab}>
          ...
          <a href={`/api/v1/download/album/${album.id}`} className={`${styles.fabBtn}`} style={{ background: "var(--bg-polaroid)", color: "var(--text-primary)" }}>
            <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
              <path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4" />
